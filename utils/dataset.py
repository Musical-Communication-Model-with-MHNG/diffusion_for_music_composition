...
def np_to_tensor(array, device):
    ...
    if len(array.shape) == 2 and array.shape[1] == 3:
        tensor = torch.zeros((2, 128, 88), dtype=torch.float32, device=device)
        for note, velocity, time in array:
            time_idx = min(int(time // (max(array[:, 2]) / 128)), 127)  # 時間を128のタイムステップにマッピング
            pitch_idx = int(note % 88)
            tensor[0, time_idx, pitch_idx] = velocity
            tensor[1, time_idx, pitch_idx] = 1
        return tensor
    else:
