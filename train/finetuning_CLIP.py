...
def run(cfg):
    ...
    train_data = clip.make_clipdata(D.image_path, D.texts)
    validation_data = clip.make_clipdata(D_val.image_path, D_val.texts)
    ...
    #train
    # print(tqdm(range(0, cfg.pretrain.train_iteration)))
    for itr in tqdm(range(0, cfg.pretrain.train_iteration)):
        # train data
        for i, data in enumerate(train_data):
            if i == len(train_data)-1:
                break
            loss_info = clip.pretrain(data, is_train=True)
        ...
        if (itr+1)%cfg.pretrain.checkpoint_interval == 0:
            ...
            params = dict()
            params["model"] = clip.get_state_dict()
            params["optimizer"] = clip.optimizer.state_dict()
            #save loss
            save_loss = dict()
            save_loss["train"] = loss_train
            save_loss["validation"] = loss_val
            params["loss"] = save_loss
            os.makedirs(f"{results_dir}/clip", exist_ok=True)
            torch.save(params, f"{results_dir}/clip/model_{itr+1}.pt")
