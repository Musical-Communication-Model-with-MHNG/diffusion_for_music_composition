...
class ProbVLM(nn.Module):
    ...
    def __init__(self, cfg, CLIP, device):
        ...
        self.net = BayesCap_for_CLIP(
            inp_dim=self.cfg.env.ProbVLM["inp_dim"], # 512
            out_dim=self.cfg.env.ProbVLM["out_dim"], # 512
            hid_dim=self.cfg.env.ProbVLM["hid_dim"], # 512
            num_layers=self.cfg.env.ProbVLM["num_layers"], # 3
            p_drop=self.cfg.env.ProbVLM["p_drop"],
