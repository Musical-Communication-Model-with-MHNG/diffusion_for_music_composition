...
class MH_naming():
    ...
    """
    def __init__(self, cfg, results_dir, model_A, model_B):
        """
    ...
    def train(self, itr):
        ...
        print("sampling")
        #sampling
        self.text_tmp = self.texts
        if self.cfg.main.n_gpus > 1:
            mp.spawn(self.__get_w_multi, nprocs=2, args=())
        else:
            self.__get_w(0)
            self.__get_w(1)
        #load
        ...
        w_A = torch.load(path_A, map_location=torch.device("cpu"), weights_only=True)["midi"]
        w_B = torch.load(path_B, map_location=torch.device("cpu"), weights_only=True)["midi"]
        ...
        img_conv_A = self.__w_to_imgconv(w_A, self.model[0].CLIP)
        img_conv_B = self.__w_to_imgconv(w_B, self.model[1].CLIP)
        ...
        for i in tqdm(range(num_samples)):
            acc_A[i], acc_B[i], like_A[i], like_B[i] = self.__cal_acceptancerate(img_conv_A[i], img_conv_B[i], self.text_conv_tmp[i])
            #accept or not
        ...
        if self.cfg.main.n_gpus > 1:
            ...
            mp.spawn(self.__finetuning_multi, nprocs=2, args=())
        else:
            print("finetuning A")
            self.__finetuning(data_A, 0)
            print("finetuning B")
            self.__finetuning(data_B, 1)
        self.__save_MH_log(itr)

    # private method
    def __w_to_imgconv(self, w_orig, clip_model):
        """
...
#---------------------- sampling diffusion ----------------------  
    ...
    def __get_w_multi(self, rank):
        ...
        os.makedirs(f"{self.results_dir}/sample", exist_ok=True)
        sample = model.sampling_diffusion()
        torch.save(sample, save_path)
    
    def __get_w(self, agent_id):
        """
        Get w from model
        ...
        os.makedirs(f"{self.results_dir}/sample", exist_ok=True)
        # params = torch.load(f"{self.results_dir}/model_current/{agent_name[agent_id]}/diffusion.pt", map_location=model.device)
...
#---------------------- acceptancerate ---------------------- 
    def __cal_acceptancerate(self, img_conv_A_orig, img_conv_B_orig, text_conv_orig):
        """
        naming game speaker to listener
        params
        ------
        img_conv_A_orig, img_conv_B_orig: torch.tensor #image_conv(w conv)
        text_conv_orig: list[str] #text(ovsevation conv)
        ...
        """
        #------- get param -------
        img_conv_A = img_conv_A_orig.clone().detach()
        img_conv_B = img_conv_B_orig.clone().detach()
        text_conv = text_conv_orig.clone().detach()
...
#---------------------- finetuning ----------------------
    # private method
    def __finetuning_multi(self, rank):
        """
        ...
        self.__save_parms(agent_id)


    def __finetuning(self, data, agent_id):
        """
        finetuning model
        ...
        #Diffusion
        for epoch in tqdm(range(self.cfg.train.finetuning_diffusion_itr), desc="Diffusion finetuning"):
            for data in diffusion_loader: # data: list of torch.tensor
                input = dict()
                input["midi"] = data[0].to(model.device)
                __ = model.train_z_model(input, is_train=True)
        # ProbVLM
        for epoch in tqdm(range(self.cfg.train.finetuning_probvlm_itr), desc="ProbVLM finetuning"):
            for data in probvlm_loader: # data: list of torch.tensor
                for d in data:
                    if type(d) == torch.Tensor:
                        print(d.shape)
                input = dict()
                input["image"] = data[0].to(model.device)
                input["text"] = data[1].to(model.device)
                __ = model.train_probVLM(input, is_train=True)
        self.__save_parms(agent_id)

    
    def __save_parms(self, agent_id):
        """
        ...
        params_diffusion["model"] = model.state_dict()
        params_diffusion["optimizer"] = model.optimizer["Diffusion"].state_dict()
    ...
    def __save_MH_log(self, itr):
        ...
        if self.cfg.main.wandb:
            wandb.log(data={"acceptance_rate_A": self.acceptance_rate[0][itr], "acceptance_rate_B": self.acceptance_rate[1][itr], "likelihood_A": self.like[0][itr], "likelihood_B": self.like[1][itr]}, step=itr)
        print(f"itr: {itr}, acceptance_rate_A: {self.acceptance_rate[0][itr]}, acceptance_rate_B: {self.acceptance_rate[1][itr]}, likelihood_A: {self.like[0][itr]}, likelihood_B: {self.like[1][itr]}")    