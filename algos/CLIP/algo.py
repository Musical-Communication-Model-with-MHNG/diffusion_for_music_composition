...
class CLIP:
    ...
    def make_clipdata(self,
        ...
        # print(f"image_conv size: {image_conv.size()}")
        # print(f"text_tokens size: {text_conv.size()}")
    ...
    def text_encoding(self,
        ...
        # print(text_token.shape)  # 例: torch.Size([40, 77])
    ...
    def image_encoding(self,
        ...
        return
        ------
        img_enc: torch.tensor encoded image
        """
        img = []
        for path in img_path:
            image = Image.open(path)
            img.append(self.preprosess(image).unsqueeze(0).to(self.device))
        img = torch.cat(img, dim=0)
        img_enc = self.model.encode_image(img).unsqueeze(1)
        return img_enc
